...
from .forms import NorthfinderCreateForms
...
def findNorth(request):
    ...
    form = NorthfinderCreateForms()

    dic_date_time = {'Date':date_1.date,'Time':date_1.time,'Azimuth':azimuth,'Elevation':altitude,'form':form}
    #html = "<html><body>It is now %s.</body></html>" % now
    #return HttpResponse(html)
    return render(request,'index.html',context=dic_date_time)

def start(request):
    messages.info(request,'قطب نما آنلاین شد','info')
    return redirect('home')

def stop(request):
    messages.warning(request,'قطب نما آفلاین شد','warning')
    return redirect('home')
